...
public class CharTypeDirective implements Directive<Integer>{
	...
	private static final Logger LOG = 
			LoggerFactory.getLogger(CharTypeDirective.class);
	...
	public boolean isMet(Integer codePoint) {
		LOG.debug("Checking if directive is met for code point: {}", 
				codePoint);
		if (matchedTypes.contains(Character.getType(codePoint))) {
			LOG.debug("CharTypeDirective met for character code point: {}",
					codePoint);
			return true;
		}
		LOG.debug("CharTypeDirective NOT met for character code point: {}",
				codePoint);
		return false;
	}
